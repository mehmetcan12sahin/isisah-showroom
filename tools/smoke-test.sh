...
[ "$(curl -s -o /dev/null -m 15 -w "%{size_download}" "$BASE/favicon.ico?$bust")" -gt 500 ] 2>/dev/null && say OK "favicon.ico dolu" || say FAIL "favicon.ico boş/yok"
n4=$(curl -s -m 15 "$BASE/olmayan-sayfa-$bust" | grep -c "Aradığınız sayfa burada değil"); [ "$n4" -ge 1 ] && say OK "404 yeni statik sayfa" || say UYARI "404 hâlâ WP teması (Plesk nginx error_page 404 /404.html gerekir)"
